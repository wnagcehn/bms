		...
		for (String key : taskSet) {
			...
			for (int i = 0; i < 2; i++) {
				...
				try {
					...
					//XxlJobLogger.log("123");
					XxlJobLogger.log("mq发送，商家id为----{0}，业务年月为----{1}，科目id为---{2}", vo.getCustomerId(),vo.getCreMonth().toString(),vo.getSubjectCode());
				} catch (Exception e) {
					XxlJobLogger.log("mq任务失败：商家id为----{0}，业务年月为----{1}，科目id为---{2}，错误信息：{}", vo.getCustomerId(),vo.getCreMonth(),vo.getSubjectCode(),e);
				}
			}
		}
	}
