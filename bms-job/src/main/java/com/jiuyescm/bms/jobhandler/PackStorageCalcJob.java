...
public class PackStorageCalcJob extends CommonCalcJob<BizPackStorageEntity,FeesReceiveStorageEntity> {
	...
	protected void calcuService(BizPackStorageEntity entity,
		...
		try{
			...
			if("succ".equals(resultVo.getSuccess())){
				XxlJobLogger.log("调用规则引擎成功   耗时【{0}】毫秒  费用【{1}】费用编号【{2}】 ",(current - start),resultVo.getPrice(),entity.getFeesNo());	
				switch(priceType){
	...
	protected void calcuStandardService(List<BizPackStorageEntity> billList) {
		for (BizPackStorageEntity entity : billList) {
	...
	private void calcu(BizPackStorageEntity entity){
		long start = System.currentTimeMillis();// 系统开始时间
