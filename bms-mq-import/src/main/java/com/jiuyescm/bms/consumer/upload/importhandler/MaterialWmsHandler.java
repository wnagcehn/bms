...
/**
  ...
  */
@Component("materialWmsHandler")
@Scope("prototype")
public class MaterialWmsHandler {

    private static final Logger logger = LoggerFactory.getLogger(MaterialWmsHandler.class);
    ...
    public void handImportFile(final String taskId) throws Exception{
        ...
        try{    
            ...
            stw.stop();
            logger.info("任务ID【{}】 -> excel读取完成，读取excel,耗时【{}】毫秒",taskId,stw.getLastTaskTimeMillis());
        }
        catch(Exception ex){
        ...
        try{
            ...
            stw.stop();
            if(k>0){
                logger.info("任务ID【{}】 -> 保存数据到正式表成功 耗时【{}】毫秒",taskId,stw.getLastTaskTimeMillis());
