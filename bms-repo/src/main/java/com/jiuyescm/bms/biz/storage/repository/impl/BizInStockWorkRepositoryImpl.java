 ...
 *
 */
@Repository("inStockWorkRepository")
public class BizInStockWorkRepositoryImpl extends MyBatisDao implements IBizInStockWorkRepository {

	private static final Logger logger = Logger.getLogger(BizInStockWorkRepositoryImpl.class.getName());
	
	@Override
	public PageInfo<BizInStockMasterEntity> query(Map<String, Object> condition,int pageNo, int pageSize) {
		List<BizInStockMasterEntity> list = selectList("com.jiuyescm.bms.biz.storage.mapper.BizInStockWorkMapper.query",condition, new RowBounds(pageNo, pageSize));
		return new PageInfo<BizInStockMasterEntity>(list);
	}

	@Override
	public int update(BizInStockMasterEntity entity) {
		return super.updateWithOne("com.jiuyescm.bms.biz.storage.mapper.BizInStockWorkMapper.update", entity);
	}
	...
	public Properties validRetry(Map<String, Object> condition) {
		...
		try{
			Object instockNo = selectOne("com.jiuyescm.bms.biz.storage.mapper.BizInStockWorkMapper.validBillForRetry", condition);
			if(instockNo != null){
			...
			instockNo = selectOne("com.jiuyescm.bms.biz.storage.mapper.BizInStockWorkMapper.validCalcuForRetry", condition);
			if(instockNo != null){
	...
	public int reCalculate(Map<String, Object> param) {
		try{
			update("com.jiuyescm.bms.biz.storage.mapper.BizInStockWorkMapper.retryForCalcu", param);
			return 1;
		}
		catch(Exception ex){
			return 0;
