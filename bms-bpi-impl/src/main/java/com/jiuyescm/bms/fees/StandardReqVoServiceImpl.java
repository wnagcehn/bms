...
public class StandardReqVoServiceImpl<T> implements IStandardReqVoService<T> {
	...
	@Resource private IPriceContractDao priceContractInfoService;
	@Resource private IPriceDispatchDao iPriceDispatchDao;
	@Resource private IPriceDispatchTemplateDao priceDispatchTemplateService;
	@Resource  private IGenericTemplateRepository genericTemplateService;
	@Resource private IPriceExtraQuotationRepository priceExtraQuotationService;
	@Resource
	private IBmsErrorLogInfoDao bmsErrorLogInfoService;
	...
	public CalcuReqVo<T> getStorageReqVo(String subjectID) {
		...
		try{
			...
			reqVo.setParams(map);
			//写入日志
			BmsErrorLogInfoEntity entity=new BmsErrorLogInfoEntity(this.getClass().getSimpleName(),Thread.currentThread().getStackTrace()[1].getMethodName(), "", ex.toString());
			bmsErrorLogInfoService.log(entity);
