...
package com.jiuyescm.bms.chargerule.payrule.web;
...
@Controller("payBillRuleController")
public class PayBillRuleController{
	@Resource
	...
	@Resource
	private IPubRuleDeliverPayableService ruleDeliverPayableService;
	@Resource
	 ...
	 */
	@DataProvider
	public void queryAll(Page<BillRulePayEntity> page,Map<String,Object> parameter){
		if(parameter!=null && "ALL".equals(parameter.get("bizTypeCode"))){
			parameter.put("bizTypeCode", "");
		}
		PageInfo<BillRulePayEntity> tmpPageInfo =payRuleService.queryAll(parameter,page.getPageNo(),page.getPageSize());
		if (tmpPageInfo != null) {
	 ...
	 */
	@DataResolver
	public String saveRule(Collection<BillRulePayEntity> datas){
		...
		try {
			...
			for(BillRulePayEntity temp:datas){
				//对操作类型进行判断
				//此为新增计费规则数据
				if(EntityState.NEW.equals(EntityUtils.getState(temp))){
					...
					PageInfo<BillRulePayEntity> tmpPageInfo =payRuleService.queryAll(condition,0,Integer.MAX_VALUE);

					if(tmpPageInfo!=null && tmpPageInfo.getList()!=null && tmpPageInfo.getList().size()>0){
						return "FALSE";
					}*/
					
					String templateNo =sequenceService.getBillNoOne(BillRulePayEntity.class.getName(), "PGR", "00000");
					...
					payRuleService.createRule(temp);
					...
					payRuleService.updateRule(temp);
				}
	 ...
	 */
	@DataResolver
	public String removeRule(BillRulePayEntity p){
		if(Session.isMissing()){
			return "长时间未操作，用户已失效，请重新登录再试！";
		}	
		try {
			...
			payRuleService.removeRule(p.getId().toString());
	...
	public List<SystemCodeEntity> queryAllService(Map<String, Object> parameter) {
		...
		if("STORAGE".equals(subjectId)){
			...
			//配送
			List<SystemCodeEntity> systemCodeList=new ArrayList<>();
			List<SystemCodeEntity> systemCodeList1 = systemCodeService.findEnumList("DISPATCH_COMPANY");
			List<SystemCodeEntity> systemCodeList2 = systemCodeService.findEnumList("DISPATCH_OTHER_SUBJECT_TYPE");
			for(int i=0;i<systemCodeList1.size();i++){
				SystemCodeEntity a=systemCodeList1.get(i);
				systemCodeList.add(a);
			}
	 ...
	 */
	@DataProvider
	public void queryRuleCustomerPayable(Page<PubRuleDeliverPayableEntity> page,Map<String,Object> parameter){
		PageInfo<PubRuleDeliverPayableEntity> pageInfo =ruleDeliverPayableService.query(parameter,page.getPageNo(),page.getPageSize());
		if (pageInfo != null) {
	 ...
	 */
	@DataResolver
	public String saveRuleCustomerPayable(PubRuleDeliverPayableEntity entity) throws Exception{
		if (null == entity) {
			throw new Exception("规则商家关系数据为空！");
		}
		//是否已存在映射关系，不存在则新增
		Map<String, Object> param = new HashMap<String, Object>();
		param.put("deliveryid", entity.getDeliveryid());
		param.put("subjectId", entity.getSubjectId());
		PubRuleDeliverPayableEntity existEntity = ruleDeliverPayableService.query(param);
		if (null == existEntity) {
			...
			int saveNums = ruleDeliverPayableService.save(entity);
			if (saveNums <= 0) {
	 ...
	 */
	@DataResolver
	public void delRuleCustomerPayable(PubRuleDeliverPayableEntity entity) throws Exception{
		if (null == entity || StringUtils.isBlank(entity.getId()+"")) {
			throw new Exception("请选择要删除的记录！");
		}
		PubRuleDeliverPayableEntity updateEntity = new PubRuleDeliverPayableEntity();
		...
		int updateNums = ruleDeliverPayableService.update(updateEntity);
		if (updateNums <= 0) {
