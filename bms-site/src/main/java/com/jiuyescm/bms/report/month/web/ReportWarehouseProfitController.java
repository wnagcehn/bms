 ...
 * 
 */
@Controller("reportWarehouseProfitController")
public class ReportWarehouseProfitController {

	private static final Logger logger = Logger.getLogger(ReportWarehouseProfitController.class.getName());

	@Resource
	private IReportWarehouseProfitService reportWarehouseProfitService;
	 ...
	 */
	@DataProvider
	public List<ReportWarehouseProfitEntity> query(Map<String, Object> param) {
		List<ReportWarehouseProfitEntity> list = reportWarehouseProfitService.query(param);
		...
		initList(list);
		List<ReportWarehouseProfitEntity> totalList=new ArrayList<ReportWarehouseProfitEntity>();

		List<List<ReportWarehouseProfitEntity>> listGroup=splitListByYear(list);//根据年份分组
		for(List<ReportWarehouseProfitEntity> group:listGroup){
			ReportWarehouseProfitEntity incomeEntity=null;
			ReportWarehouseProfitEntity costEntity=null;
			ReportWarehouseProfitEntity manageEntity=null;
			ReportWarehouseProfitEntity profitEntity=null;
			for(ReportWarehouseProfitEntity entity:group){
	...
	private void initList(List<ReportWarehouseProfitEntity> list){
		for(ReportWarehouseProfitEntity entity:list){
	 ...
	 * @return
	 */
	private ReportWarehouseProfitEntity getRealCostEntity(
			ReportWarehouseProfitEntity costEntity,
			ReportWarehouseProfitEntity manageEntity) {
		ReportWarehouseProfitEntity realCostEntity=new ReportWarehouseProfitEntity();
		realCostEntity.setWarehouseCode(costEntity.getWarehouseCode());
		realCostEntity.setWarehouseName(costEntity.getWarehouseName());
	 ...
	 * @return
	 */
	private ReportWarehouseProfitEntity getprofitEntity(
			ReportWarehouseProfitEntity incomeEntity,
			ReportWarehouseProfitEntity costEntity,
			ReportWarehouseProfitEntity manageEntity, boolean isCheck) {
		ReportWarehouseProfitEntity entity=new ReportWarehouseProfitEntity();
		...
		entity.setWarehouseCode(incomeEntity.getWarehouseCode());
		entity.setWarehouseName(incomeEntity.getWarehouseName());
		return entity;
	}

	private ReportWarehouseProfitEntity getDefaultEntity(ReportWarehouseProfitEntity model, int feesType) {
		ReportWarehouseProfitEntity entity=new ReportWarehouseProfitEntity();
		entity.setReportYear(model.getReportYear());
		entity.setWarehouseCode(model.getWarehouseCode());
	...
	private List<List<ReportWarehouseProfitEntity>> splitListByYear(
			List<ReportWarehouseProfitEntity> list) {
		List<List<ReportWarehouseProfitEntity>> groupList=new ArrayList<List<ReportWarehouseProfitEntity>>();
		Map<String,List<ReportWarehouseProfitEntity>> map=Maps.newLinkedHashMap();
		for(ReportWarehouseProfitEntity entity:list){
			if(map.containsKey(entity.getWarehouseCode())){
				List<ReportWarehouseProfitEntity> mapList=map.get(entity.getWarehouseCode());
				mapList.add(entity);
				map.put(entity.getWarehouseCode(), mapList);
			}else{
				List<ReportWarehouseProfitEntity> mapList=new ArrayList<ReportWarehouseProfitEntity>();
				mapList.add(entity);
				map.put(entity.getWarehouseCode(), mapList);
			}
		}
